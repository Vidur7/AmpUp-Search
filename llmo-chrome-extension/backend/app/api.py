...
async def analyze_webpage(
    ...
    try:
        ...
        # Extract data from result and create Analysis record
        try:
            logger.info(f"Processing analysis result for {request.url}")

            # Extract data safely from the result
            data = result.get(
                "data", result
            ...
            logger.info(f"Extracted data - Score: {overall_score}, URL: {request.url}")
            ...
            from app.models import AnonymousUsage
            ...
            logger.error(
                f"Error processing analysis result for {request.url}: {str(e)}",
                exc_info=True,
            )
            return {
                "success": False,
                "error": "Internal server error: Failed to process analysis result",
                "message": "The analysis completed but failed to save the results",
                "data": create_error_response(
                    str(request.url),
                    "Internal server error: Failed to process analysis result",
                ),
            }
